--[[
lua/weapons/csgo_baseknife.lua
--]]
AddCSLuaFile()

local TTT = ( GAMEMODE_NAME == "terrortown" or cvars.Bool("csgo_knives_force_ttt", false) )

local BaseClass = baseclass.Get( TTT and "weapon_tttbase" or "weapon_base" )
...
if ( SERVER ) then
  ...
  CreateConVar("csgo_knives_dmg_sec_back", 400, FCVAR_ARCHIVE, "How much damage deal when hit with secondary attack from behind")
  CreateConVar("csgo_knives_dmg_sec_front", 150, FCVAR_ARCHIVE, "How much damage deal when hit with secondary attack in front or from side")
  CreateConVar("csgo_knives_dmg_prim_back", 130, FCVAR_ARCHIVE, "How much damage deal when hit with primary attack from behind")
  CreateConVar("csgo_knives_dmg_prim_front1", 120, FCVAR_ARCHIVE, "How much damage deal when firstly hit with primary attack in front or from side")
  CreateConVar("csgo_knives_dmg_prim_front2", 110, FCVAR_ARCHIVE, "How much damage deal when subsequently hit with primary attack in front or from side")
end
...
SWEP.Category              = "CS:GO M9 Custom Knifes"
...
function SWEP:Initialize()
  self:SetHoldType("knife") -- Avoid using SetWeaponHoldType! Otherwise the players could hold it wrong!
end
...
function SWEP:Deploy()
  ...
  self:EmitSound("weapons/imbrokeru_knife/knife_deploy1.wav", 50, 110 )
  self:SetIdleTime( CurTime() + self.Owner:GetViewModel():SequenceDuration() )
...
function SWEP:DoAttack( Altfire )
  ...
  if tr.Hit and ( not (IsValid(tr.Entity) and tr.Entity) or tr.HitWorld ) then
    self:EmitSound( "weapons/imbrokeru_knife/knife_hitwall1.wav", 100, 100)
  ...
  if HitEntity:IsPlayer() then
    timer.Simple(0.1, function()
      if SERVER then
		...
		self:EmitSound( "weapons/imbrokeru_knife/knife_hit3.wav", 100, 100)
	end
  ...
  local RMB_FRONT  = 250
  local LMB_BACK   = 350
  local LMB_FRONT1 = 230
  local LMB_FRONT2 = 340
