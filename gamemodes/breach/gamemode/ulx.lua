--[[
gamemodes/breach/gamemode/ulx.lua
--]]
if !ulx or !ULib then
...
function ulx.forcespawn( ply, plys, class, silent )
	...
	if cl and gr then
		...
		if ( istable( plys ) ) then
			...
			if ( plys:GetNActive() ) then



				plys:SetupNormal()

				plys:ApplyRoleStats( cl )

				if pos then

					plys:SetPos( table.Random( pos ) )
...
function ulx.forcescp( plyc, plyt, scp, silent )
	...
	for k, v in pairs( SPCS ) do
		...
		if v.name == scp then
			plyt.JustSpawned = true
			v.func( plyt )
			plyt:Give("hacking_doors_scp")
			plyt.JustSpawned = false
			if silent then
