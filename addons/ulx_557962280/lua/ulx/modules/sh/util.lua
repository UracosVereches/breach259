...
function ulx.globalbanid( calling_ply, steamid, reason )
	steamid = steamid:upper()
	if not ULib.isValidSteamID( steamid ) then
	...
	if !IsValid(player.GetBySteamID(steamid)) then
		ULib.tsayError( calling_ply, "Игрока нету на сервере." )
		return
	end
	...
	-- Delay by 1 frame to ensure any chat hook finishes with player intact. Prevents a crash.
	GlobalBan(steamid, reason)
	timer.Simple(4, function()
		ULib.queueFunctionCall( ULib.addBan, steamid, 0, "Ksaikok Anticheat: Блокировка администрацией - "..reason, name, calling_ply )
	end)
end
local globalbanid = ulx.command( CATEGORY_NAME, "ulx globalbanid", ulx.globalbanid, nil, false, true )
globalbanid:addParam{ type=ULib.cmds.StringArg, hint="steamid" }
globalbanid:addParam{ type=ULib.cmds.StringArg, hint="reason", ULib.cmds.optional, ULib.cmds.takeRestOfLine, completes=ulx.common_kick_reasons }
globalbanid:defaultAccess( ULib.ACCESS_ADMIN )
globalbanid:help( "Глобальная блокировка человека с отслеживанием альтернативных аккаунтов. ИСПОЛЬЗОВАТЬ ТОЛЬКО ДЛЯ ПЕРМАНЕНТНЫХ БЛОКИРОВОК И В ПРИСУТСТВИИ БЛОКИРУЕМОГО ИГРОКА НА СЕРВЕРЕ!" )
