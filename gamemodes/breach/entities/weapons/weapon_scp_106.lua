...
function SWEP:Think()
	...
	if SERVER then
		for k, v in pairs( self.SoundPlayers ) do
			if v.ply:GTeam() == TEAM_SPEC or v.ply:GTeam() == TEAM_SCP or v.ply:GTeam() == TEAM_DZ or ( v.time and v.time < CurTime() ) or self.Owner:GetPos():DistToSqr( v.ply:GetPos() ) > 562500 then
				--[[
				net.Start( "SendSound" )
					net.WriteInt( 0, 2 )
		...
		for k, v in pairs( e ) do
			if IsValid( v ) and  v:IsPlayer() then
				if  v:GTeam() != TEAM_SPEC and v:GTeam() != TEAM_SCP and v:GTeam() != TEAM_DZ then
	...
	if self.Owner:Health() <= 100 then
		...
		timer.Simple(5, function() self.Owner:Kill() end)
	end
end
	...
	if SERVER then
		...
		if IsValid(ent) then
			if ent:IsPlayer() then
				...
				if timer.Exists("Vizmrenie!" ..ent:SteamID64()) then return end
				...
				ent:EmitSound( "Place" )
				--self.TPPoint = ent:GetPos()
				--ent:TeleportSequence(  )
				ent:Freeze( true )
				self.Owner:SetNWInt("EXP", self.Owner:GetNWInt("EXP") + 1)
				self:GetOwner():AddExp(10, true)
				ent:ScreenFade( SCREENFADE.OUT, Color( 0, 0, 0, 255 ), 1, 5 )
				ent:SendLua('surface.PlaySound("sfx/scp/106/laugh.ogg")')
				local health = math.Clamp( self.Owner:Health() + 100, self.Owner:Health(), self.Owner:GetMaxHealth() )
				--self.Owner:SetHealth( health )
				util.Decal( "Decal106", tr1.HitPos - tr1.HitNormal, tr1.HitPos + tr1.HitNormal )
				timer.Create( "poymal" ..ent:SteamID64(), 0.1, 38, function()
		            if IsValid( self ) and IsValid( ent ) then
			            if i % 20 == 0 then
				            self:SendSound( self.Disappear, 500 )
			            end
			            ent:SetPos( ppos - Vector( 0, 0, 2 * ( 40 - i ) ) )
		            end
		            i = i - 1
	            end )
				--[[if pos then
				...
				timer.Create( "Vizmrenie!" ..ent:SteamID64(), 6, 1, function()

					    roundstats.teleported = roundstats.teleported + 1
					    ent:TakeDamage( math.random( 30, 50 ), self.Owner, self.Owner )
				        ent:SetPos(Vector(2421, 4650, 537))
						ent:SetAngles( ang )
				        ent:Freeze( false )
...
function SWEP:SecondaryAttack()
	if SERVER then
		if self.NextPlace > CurTime() then
			self.Owner:PrintMessage( HUD_PRINTTALK, "Вы должны подождать "..math.ceil( self.NextPlace - CurTime() ).." перед след использованием!" )
			return
		end
		self.NextPlace = CurTime() + 30
...
--[[
hook.Add("DoPlayerDeath", "106dies", function(ply)
    if ply:GetNClass() != ROLES.ROLE_SCP106 then return end
	...
	timer.Simple(4, function() ply:GodDisable() end)
end)
]]
SWEP.NextTP = 0
