--[[
lua/weapons/weapon_sh_percussiongrenade.lua
--]]
AddCSLuaFile()

if CLIENT then
	SWEP.WepSelectIcon 	= surface.GetTextureID("vgui/entities/grenade_frag")
...
game.AddAmmoType({name = "Percussion Grenades"})

SWEP.Primary.ClipSize = 1
SWEP.Primary.DefaultClip = 1
SWEP.Primary.Ammo = "Percussion Grenades"
...
SWEP.ThrownProjectile = "proj_sh_percussiongrenade"
SWEP.ThrowDelay = 0.2

SWEP.AllowCooking = false
SWEP.CookTime = 9999
SWEP.ThrowForce = 1250

function SWEP:SetupVars()
	self.ThrowForce = PERCUSSIONGRENADE_SETTINGS.ThrowForce
	self.detonate_timer = self.CookTime

	if (!PERCUSSIONGRENADE_SETTINGS.AllowAmmo) then
...
if (SERVER) then
	...
	function SWEP:PreDrawViewModel(vm)
		if (vm) then
			vm:SetSubMaterial(0, "models/weapons/v_models/eq_percussiongrenade/percussiongrenade")
		end
	end
