--[[
gamemodes/breach/entities/weapons/wep_jack_job_drpradio.lua
--]]
AddCSLuaFile()

if CLIENT then
	SWEP.WepSelectIcon 	= surface.GetTextureID("vgui/entities/radio")
...
--[[hook.Add("PlayerButtonDown", "RadioOFFON", function(ply, but)
  if !ply:HasWeapon("wep_jack_job_drpradio") then return end
  if but != KEY_G then return end
  print(ply)
	print("okay")
  local radio = ply:GetWeapon("wep_jack_job_drpradio").Enabled
  if but == KEY_G then
...
function SWEP:DrawHUD()
	...
	if self.Enabled then
		showtext = "Канал рации " .. self.Channel
		showcolor = Color(0, 255, 0)
	else
		showtext = "Рация выключена"
