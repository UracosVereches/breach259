...
function ENT:Use(activator, caller)
	if activator:GetNClass() == ROLES.ROLE_Engi then
		self:SetReady(!self:GetReady())
	elseif activator:GTeam() == TEAM_SCP and IsFirstTimePredicted() then
		self:TakeDamage(15, activator, activator)
	end
end
