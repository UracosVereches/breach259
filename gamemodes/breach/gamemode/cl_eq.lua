--[[
gamemodes/breach/gamemode/cl_eq.lua
--]]
EQHUD = {
...
local selectioncolor = Color(150, 150, 150, 230)
...
function DrawEQ()
	...
	surface.SetDrawColor( Color( 10, 10, 10, 0 ) )
	...
	for i = 0, EQHUD.width * EQHUD.height - 1 do
		...
		draw.Text( {
			...
			pos = { sx + EQHUD.c_offset + 150, sy + hi - 32 },
		...
		if IsValid( EQHUD.weps[i + 1] ) then
			...
			if EQHUD.weps[i + 1].IconLetter then
				...
				cam.PushModelMatrix( mx )
					...
					draw.SimpleText( letter, usefont, x + EQHUD.c_dim / 2 * 1.25, y + EQHUD.c_dim / 2, Color(255, 210, 0, 90 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )

				cam.PopModelMatrix()

			--elseif EQHUD.weps[i + 1].CW20Weapon then
				
				--local ico = "materials/entities/"..EQHUD.weps[i + 1]:GetClass() or 0

				--surface.SetTexture( ico )

				--surface.SetDrawColor( Color( 255, 255, 255, 255 ) )

				--surface.DrawTexturedRect( x + 2, y + EQHUD.c_dim / 18 + -6, EQHUD.c_dim - 4, EQHUD.c_dim / 1 - 4 )
			...
			QuickButton( x, y, EQHUD.c_dim, EQHUD.c_dim, function()
				...
				LocalPlayer():DropWeapon( EQHUD.weps[i + 1] )
				...
				if SERVER then
					...
					LocalPlayer():DoAnimationEvent(ACT_GMOD_GESTURE_ITEM_THROW)
				end
...
function PaintScreenTip()
	...
	draw.Text( {
		...
		pos = { screen_tip.pos[1] + 14, screen_tip.pos[2] + 4 },
...
function DrawInfoBox( w, h )
	...
	local tops = { "Автор:", "Поддержка:", "Использование:", "История/инструкция:" }
