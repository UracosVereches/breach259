--[[
lua/weapons/weapon_sh_healgrenade.lua
--]]
AddCSLuaFile()

if CLIENT then
	SWEP.WepSelectIcon 	= surface.GetTextureID("vgui/entities/grenade_health")
	SWEP.BounceWeaponIcon = false

	SWEP.PrintName = "Лечащая граната"
	SWEP.BounceWeaponIcon = false
...
game.AddAmmoType({name = "Heal Grenades"})

SWEP.Primary.ClipSize = 1
SWEP.Primary.DefaultClip = 1
SWEP.Primary.Ammo = "Heal Grenades"
...
SWEP.ThrownProjectile = "proj_sh_healgrenade"
SWEP.ThrowDelay = 0.2
...
function SWEP:SetupVars()
	self.AllowCooking = HEALGRENADE_SETTINGS.AllowCooking
	self.CookTime = HEALGRENADE_SETTINGS.CookTime
	self.ThrowForce = HEALGRENADE_SETTINGS.ThrowForce
	self.detonate_timer = self.CookTime

	if (!HEALGRENADE_SETTINGS.AllowAmmo) then
...
if (SERVER) then
	...
	function SWEP:PreDrawViewModel(vm)
		if (vm) then
			vm:SetSubMaterial(0, "models/weapons/v_models/eq_healgrenade/healgrenade")
		end
	end
