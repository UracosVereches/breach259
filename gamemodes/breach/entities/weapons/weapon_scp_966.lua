	...
	if SERVER then
		...
		if IsValid(ent) then
			if ent:IsPlayer() then
				...
				if ent:Alive() then
					ent:TakeDamage( math.random(30, 45), self.Owner, self.Owner )
					ent:SendLua('surface.PlaySound("weapons/knife_stab.wav")')
					self:GetOwner():SendLua('surface.PlaySound("weapons/knife_stab.wav")')
			end
			else
				if ent:GetClass() == "func_breakable" and !ent:IsPlayer() then
					ent:TakeDamage( 100, self.Owner, self.Owner )
					ent:EmitSound("weapons/knife_stab.wav")
				end
			end
		end
	end
