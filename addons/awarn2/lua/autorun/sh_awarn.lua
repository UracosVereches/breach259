...
CreateConVar( "awarn_decay", "0", bit.bxor( FCVAR_ARCHIVE, FCVAR_NOTIFY, FCVAR_REPLICATED ), "If enabled, active warning acount will decay over time. 1=Enabled 0=Disabled" )
...
CreateConVar( "awarn_logging", "1", bit.bxor( FCVAR_ARCHIVE, FCVAR_NOTIFY, FCVAR_REPLICATED ), "If enabled, AWarn will log actions to a data file. 1=Enabled 0=Disabled" )
...
function awarn_ULXCompatability()
	...
	if ulx then
		...
		ULib.ucl.registerAccess( "awarn_remove", ULib.ACCESS_SUPERADMIN, "Ability to reduce a player's active warnings.", "AWarn" )
...
function awarn_checkadmin_view( self )
	...
	if ulx then
		...
		if self:IsSuperAdmin() or self:IsAdmin() or self:IsUserGroup("spectator") then return true end
	elseif evolve then
...
function awarn_checkadmin_remove( self )
	...
	if ulx then
		...
		if self:IsSuperAdmin() or self:IsUserGroup("spectator") then return true end
	elseif evolve then
