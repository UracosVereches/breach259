...
function SetupPlayers()
	...
	if plycount < 10 then
		local scpply = allplayers[math.random(1, #allplayers)]
		if scp == SCP_173 then
			scpply:SetSCP173()
		...
		for i=1,math.Round(#allplayers / 3) do
			local rndplayer = allplayers[math.random(1, #allplayers)]
		...
		for k,v in pairs(gteams.GetPlayers(TEAM_CLASSD)) do
			v:SetPos(SPAWN_CLASSD[math.random(1, #SPAWN_CLASSD)])
		end
		for k,v in pairs(gteams.GetPlayers(TEAM_GUARD)) do
			v:SetPos(SPAWN_GUARD[math.random(1, #SPAWN_GUARD)])
		end
		return
	end
